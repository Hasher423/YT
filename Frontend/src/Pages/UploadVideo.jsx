...
//   /* -------------------- UPLOAD FUNCTIONS -------------------- */
//  const uploadResumable = async (file, type) => {
//   // 1. Get signature from backend
//   const sigRes = await axios.get(`${import.meta.env.VITE_BACKEND_URI}/video/signUpload?type=${type}`);
//   const { signature, timestamp, apiKey, cloudName, folder, filename } = sigRes.data;
...
//     await axios.post(`https://api.cloudinary.com/v1_1/${cloudName}/${type}/upload`, formData, {
//       headers: {
//         'Content-Type': 'multipart/form-data',
//         'Content-Range': `bytes ${start}-${end - 1}/${file.size}`,
//       },
//       onUploadProgress: (e) => {
//         const percent = Math.round(((start + e.loaded) * 100) / file.size);
//         dispatch({ type: 'SET_PROGRESS', progress: percent });
//       },
//       timeout: 0,
//     });
...
//     console.log(start )
//     localStorage.setItem('upload', start.toString())
//   }


//   // Return final file info
//   return {
//     public_id: filename,
//     folder,
//   };
...
// const handleSubmit = async (e) => {
//   e.preventDefault();
//   if (!state.video || !state.thumbnail) return;
...
//     const videoResponse = await uploadResumable(state.video, 'video');

//     // Upload thumbnail (small, can be single request)
//     const thumbnailResponse = await uploadResumable(state.thumbnail, 'image');

//     // Save metadata to backend
//     await axios.post(`${import.meta.env.VITE_BACKEND_URI}/video/upload`, {
//       videoResponse,
//       thumbnailResponse,
//       title: state.title,
...
//         <div className="absolute inset-0 bg-black/70 flex flex-col items-center justify-center z-50">
//           <Loader size="lg" />
//           <p className="mt-4 text-xl text-white animate-pulse">Finalising upload… please wait</p>
//         </div>
//       )}

//       {state.isUploaded && (
//         <div className="text-center space-y-6">
//           <h1 className="text-4xl font-bold text-white">Video uploaded successfully!</h1>
//           <button
//             onClick={() => navigate('/')}
//             className="px-6 py-3 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition"
//           >
...
//           {/* Error */}
//           {state.error && (
//             <p className="text-red-500 bg-red-900/30 p-3 rounded text-center">{state.error}</p>
//           )}
...
//               <button
//                 type="submit"
//                 className="px-8 py-3 rounded font-medium transition bg-emerald-600 hover:bg-emerald-700 text-white"
//               >
//                 Upload Video
//               </button>
//             </div>
